...
pub struct WorkflowService {
    #[allow(dead_code)]
    _planner: Arc<dyn PlannerProgress + Send + Sync>,
    memory: Arc<Mutex<Box<dyn MemoryStorage>>>,
    #[allow(dead_code)]
    _tools: Arc<Mutex<Box<dyn crate::tool::ToolManagerService>>>,
    #[allow(dead_code)]
    _llm: Arc<dyn crate::llm::Llm + Send + Sync>,
...
impl WorkflowService {
    ...
    pub fn start(&self, plan: ExecutionPlan) -> RavenResult<String> {
        ...
        let mut plan_guard = self.plan.lock().map_err(|e| RavenError::LockPoisoned(e.to_string()))?;
        *plan_guard = Some(plan.clone());

        let mut status_guard = self.status.lock().map_err(|e| RavenError::LockPoisoned(e.to_string()))?;
        status_guard.update_state(WorkflowState::Pending);
        status_guard.mark_current_step(None, None);
        drop(status_guard);

        let _ = self.event_bus.publish(AgentEvent::WorkflowStarted { workflow_id: workflow_id.clone() });
        let result = self.execute(plan);
        let _ = self.event_bus.publish(AgentEvent::WorkflowFinished { workflow_id, result: result.as_ref().map(|s| s.clone()).map_err(|e| e.to_string()) });
        result
    }

    pub fn pause(&self) -> RavenResult<()> {
        let mut status_guard = self.status.lock().map_err(|e| RavenError::LockPoisoned(e.to_string()))?;
        if status_guard.state == WorkflowState::Running || status_guard.state == WorkflowState::Waiting {
            status_guard.update_state(WorkflowState::Waiting);
            Ok(())
        } else {
            Err(RavenError::Workflow(format!("cannot pause workflow in state {:?}", status_guard.state)))
        }
    }
    ...
    pub fn resume(&self) -> RavenResult<()> {
        ...
        if status_guard.state == WorkflowState::Waiting {
            ...
            Err(RavenError::Workflow(format!("cannot resume workflow in state {:?}", status_guard.state)))
        }
    }

    pub fn cancel(&self) -> RavenResult<()> {
        let mut status_guard = self.status.lock().map_err(|e| RavenError::LockPoisoned(e.to_string()))?;
        if status_guard.state != WorkflowState::Completed && status_guard.state != WorkflowState::Cancelled {
            status_guard.update_state(WorkflowState::Cancelled);
            Ok(())
        } else {
            Err(RavenError::Workflow(format!("cannot cancel workflow in state {:?}", status_guard.state)))
        }
    }

    pub fn retry(&self) -> RavenResult<String> {
        let current_plan = self.plan.lock().map_err(|e| RavenError::LockPoisoned(e.to_string()))?.clone();
        if let Some(plan) = current_plan {
            ...
            Err(RavenError::Workflow("no workflow plan available to retry".into()))
        }
    }
    ...
    pub fn continue_workflow(&self) -> RavenResult<String> {
        ...
        if let Some(plan) = current_plan {
            ...
            Err(RavenError::Workflow("no workflow plan available to continue".into()))
        }
    }
    ...
    fn execute(&self, plan: ExecutionPlan) -> RavenResult<String> {
        ...
        let manager = self.memory.lock().map_err(|e| RavenError::LockPoisoned(e.to_string()))?;
